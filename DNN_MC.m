...
combset = 3; % number of combinations
shufflenum = 100; % 1 cell prediction→100、other→10
...
% 4cells
shuffle_mean = [mean(RMSE_corr_4c1_s(:,1:10),2) mean(RMSE_corr_4c1_s(:,11:20),2) mean(RMSE_corr_4c1_s(:,21:30),2) mean(RMSE_corr_4c1_s(:,31:40),2)];

% 5cells
shuffle_mean = [mean(RMSE_corr_5c1_s(:,1:10),2) mean(RMSE_corr_5c1_s(:,11:20),2) mean(RMSE_corr_5c1_s(:,21:30),2) mean(RMSE_corr_5c1_s(:,31:40),2) mean(RMSE_corr_5c1_s(:,41:50),2)];
...
RMSE_corr_3c1(index_exclude,:)=[];
RMSE_corr_3c1_s(index_exclude,:)=[];
...
RMSE_corr_cv(index_exclude,:)=[];
% RMSE_corr_cv_s(index_exclude,:)=[];
RMSE_corr_4c1(index_exclude,:)=[];
RMSE_corr_4c1_s(index_exclude,:)=[];
...
RMSE_corr_cv(index_exclude,:)=[];
% RMSE_corr_cv_s(index_exclude,:)=[];
RMSE_corr_5c1(index_exclude,:)=[];
RMSE_corr_5c1_s(index_exclude,:)=[];
...
% 3cells
shuffle_mean = [mean(RMSE_corr_3c1_s(:,1:10),2) mean(RMSE_corr_3c1_s(:,11:20),2) mean(RMSE_corr_3c1_s(:,21:30),2)];
...
for i = 1:size(shuffle_mean,1)
    for j= 1:size(shuffle_mean,2)
        R1_real(i,j) = shuffle_mean(i,j) - RMSE_corr_5c1(i,j); 
        R1_real_rate(i,j) = R1_real(i,j)/shuffle_mean(i,j);
    end
end
...
% 小さい⇔大きいで青⇔赤【jet】になるようにRGBをつくる
% 参考：https://jp.mathworks.com/matlabcentral/answers/312777-how-can-i-retrieve-rgb-array-from-a-colormap-given-a-cdata
% X = [1:1:size(x_test,1)]';
% h=imagesc('CData',X);
% CData=h.CData;
% cmap = jet(256);
...
% 
% figure;
% subplot(4,1,1);gscatter(Y(:,1),Y(:,2),ind1,RGB_stack,'.',8,'off');title('cell #1');xlabel('MDS1');ylabel('MDS2');
% subplot(4,1,2);gscatter(Y(:,1),Y(:,2),ind2,RGB_stack,'.',8,'off');title('cell #2');xlabel('MDS1');ylabel('MDS2');
% subplot(4,1,3);gscatter(Y(:,1),Y(:,2),ind3,RGB_stack,'.',8,'off');title('cell #3');xlabel('MDS1');ylabel('MDS2');
% subplot(4,1,4);gscatter(Y(:,1),Y(:,2),ind4,RGB_stack,'.',8,'off');title('cell #4');xlabel('MDS1');ylabel('MDS2');
...
pvalue_all=[Pv(mfao1) Pv(mfao2) Pv(mfao3) Pv(mfao4)]'% Pv(mfao4) Pv(mfao5)
