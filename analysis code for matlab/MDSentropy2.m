% MDSentropy2.m
% 220926
% for digitalized value 
% R1_real_rate → RMSE_judge_Nc1 (N; number of cells recorded simultaneously)

function [pvalue] = MDSentropy2(RMSE_judge_4c1,Idx);

RMSE_judge = RMSE_judge_4c1;

% to calculate entropy change 0 → 0.1
RMSE_judge_1 = RMSE_judge(:,1);
RMSE_judge_1(RMSE_judge_1==0)=0.1;
% 
% RMSE_judge_2 = RMSE_judge(:,2);
% RMSE_judge_2(RMSE_judge_2==0)=0.1;
% 
% RMSE_judge_3 = RMSE_judge(:,3);
% RMSE_judge_3(RMSE_judge_3==0)=0.1;
% 
% RMSE_judge_4 = RMSE_judge(:,4);
% RMSE_judge_4(RMSE_judge_4==0)=0.1;
% 
% RMSE_judge_5 = RMSE_judge(:,5);
% RMSE_judge_5(RMSE_judge_5==0)=0.1;

% REAL entropy
clear ch_rate1 ch_rate1_sum ch_rate1_sum ch_rate1_sumsum P1 H1 H1_sum
ch_rate1 = RMSE_judge_1(Idx);
...
% SHUFFLE entropy
clear I
I = [1:size(RMSE_judge,1)]';
%IDX={};
shuffle_n = 100;

for j=1:shuffle_n
    for i=1:size(RMSE_judge,1)
        Idx_s(i,:) =randperm(size(RMSE_judge,1),4);
    end
    clear i
    Idx_s =[I Idx_s];
    ...
    for i=1:size(RMSE_judge,1)
        ...
        if Idx_s(i,1) == Idx_s(i,2)
            Idx_s(i,2) = randperm(size(RMSE_judge,1),1);
        elseif Idx_s(i,1) == Idx_s(i,3)
            Idx_s(i,3) = randperm(size(RMSE_judge,1),1);
        elseif Idx_s(i,1) == Idx_s(i,4)
            Idx_s(i,4) = randperm(size(RMSE_judge,1),1);
        elseif Idx_s(i,1) == Idx_s(i,5)
            Idx_s(i,5) = randperm(size(RMSE_judge,1),1);
        end
...
entropy_s_inf95_1=entropy_s_sort1(shuffle_n*0.05);
% entropy_s_inf95_2=entropy_s_sort2(shuffle_n*0.05);
% entropy_s_inf95_3=entropy_s_sort3(shuffle_n*0.05);
% entropy_s_inf95_4=entropy_s_sort4(shuffle_n*0.05);
% entropy_s_inf95_5=entropy_s_sort5(shuffle_n*0.05);
% entropy_s_inf95 = [entropy_s_inf95_1;entropy_s_inf95_2;entropy_s_inf95_3];

entropy_s_sup95_1=entropy_s_sort1(shuffle_n-shuffle_n*0.05);
% entropy_s_sup95_2=entropy_s_sort2(shuffle_n-shuffle_n*0.05);
% entropy_s_sup95_3=entropy_s_sort3(shuffle_n-shuffle_n*0.05);
% entropy_s_sup95_4=entropy_s_sort4(shuffle_n-shuffle_n*0.05);
% entropy_s_sup95_5=entropy_s_sort5(shuffle_n-shuffle_n*0.05);
% entropy_s_sup95 = [entropy_s_sup95_1;entropy_s_sup95_2;entropy_s_sup95_3];
...
clear ao1 ao2 ao3 ao4 ao5 fao1 fao2 fao3 fao4 fao5 mfao1 mfao2 mfao3 mfao4 mfao5 pvalue
