%% ripple_candidates_detection_ao.m
%% 200206 update
%% Blackrockで記録したLFPからリップルをdetectする
...
% ns6のデータをmatlabに読み込む
% openNSx
% % これで読み込みたいns6のデータを読み込む。
% % MClustとOpen t file after MClustがパスに追加されてないと’未定義’になるので注意する
% lfp=double(NS6.Data(33,:))*0.1526; % 1~32まではシリコンプローブのチャネル
% time=[1:1:NS6.MetaTags.DataPoints];
% time=time/30000;
% lfp=lfp'; time=time';
% % シリコンプローブで記録したチャネルのLFPを見たければ、
% %lfp=double(NS6.Data(1,:))*0.25; % とする。DAM80経由で記録したチャネルと掛ける数値が異なるため注意する
% 
% % 前処理追加（20200303）
% % DAM80を用いて、blackrockにつなげて記録すると、たまに変なノイズ（大きなスパイク？）がのるため、リップル検出のSDに影響する恐れがあるので、予め除く。
% pks=find(lfp>3000);
% lfp(pks)=lfp(pks-1); % ノイズ部分をノイズ前の数値に置換する
% figure; plot(time,lfp); % ノイズとれたか確認用

function ripple_candidates_detection_ao(basepath,time,lfp0)
tic;
cd(basepath);
...
Fmax = 249; % フィルタ周波数最大値
Fmin = 120; % フィルタ周波数最小値
ThrSD = 3; % RMSの閾値（ベースラインから何SDか）)open
...
atfname='200129002';
% time=t';
% lfp0=lfp'; %LFPが含まれる列を指定（LFPの波形）
fs=round(1/(time(2)-time(1))); % sampling frequency

%% Set file names
filenamebase  = strcat(atfname,'_',num2str(Fmin),'-',num2str(Fmax),'Hz_',num2str(ThrSD),'SD');
...
%     AllCandidates.rms_y      = rms_y;
   ...
   % AllSettings.ii    = ii;
    ...
    clearvars -except AllCandidates AllSettings basepath lfp0 time
