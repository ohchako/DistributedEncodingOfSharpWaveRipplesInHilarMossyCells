...
function [spwpeakpos, spwpeaktime, spwpeakamp, spwnum, spwfrq, spwonsetpos, spwend, spwdur, y1sw]=ao_SW_detection_in_vitro2(y1,t);
% spwpeakamp (μV)
% spwpeaktime (s)
% spwfrq (Hz)
...
for j=1:numel(spwpeakpos); % ピークから50ms前までのfiltered traceにおいて、baseline+sdを超えた点をonsetとする
    AA=[round(spwpeakpos(j)-0.05*fs):round(spwpeakpos(j))]; %ピークから50ms前までのインデックス（50msにしたのは適当だが、この範囲内でonsetを見つけられそう）
...
for j=1:numel(spwpeakpos); % ピークから50ms後までのfiltered traceにおいて、peakposからbaseline+SDに戻った点をend
    A2=[round(spwpeakpos(j)):round(spwpeakpos(j)+0.1*fs)]; %ピークから50ms後までのインデックス（50msにしたのは適当だが、この範囲内でendを見つけられそう）
...
%figure; plot(peakpos./fs./60, peakamp,'.');%横軸を分にする
...
%subplot(212);
plot(t,y1-0.1,'k'); xlim([0 t(end)]); % axis tight; hold on;
