...
function ripple_finalization2_ao(basepath)
tic; cd(basepath); %clearvars -except basepath
%ARtime=ACtime; ARlfp0=AClfp0;
%% Set patameters
...
%% Detect ripple amplitudes (201009 Ayako)
% filtered traceにおけるリップルのオンセット・オフセットのindexを、raw traceにおいて検出し、raw
% traceにおける最小・最大値の差をripple amplitudeとした

for i=1:allripplenum
    R(1,i)={ARlfp0(ripplefinalindex2(i,2):ripplefinalindex2(i,4))};
    R(2,i)={ripplefinalindex2(i,2):ripplefinalindex2(i,4)};
    Rmaxind(i)=find(R{1,i}==max(R{1,i}),1,'first');
    Rminind(i)=find(R{1,i}==min(R{1,i}),1,'first');
    % 最小・最大値となるindexをとってくる(Rmaxind_all,Rminind_all)
    r=R{2,i};
    Rmaxind_all(:,i)=r(Rmaxind(i));
    Rminind_all(:,i)=r(Rminind(i));
    clear r
    rippleamp(i,:)=ARlfp0(Rmaxind_all(i))-ARlfp0(Rminind_all(i));
end
% raw traceにおいてどの部分を最小・最大ととってきたかを確認する図
% figure;
% plot(ARtime,ARlfp0,'k',...
%     ARtime(Rmaxind_all),ARlfp0(Rmaxind_all),'ro',...
%     ARtime(Rminind_all),ARlfp0(Rminind_all),'gs');
...
RippleInfo2=[ripplefinalindex2,ripplefinaltiming2,...
             ...
             rippleamp,...
             Zripplerms(ripplefinalindex2(:,3))]; % この時点では14列にしておく。→15列にした（201008）
%  1列　リップル番号
%  2列　オンセットのインデックス
%  3列　ピークのインデックス
%  4列　オフセットのインデックス
...
% end
% title(['Raw LFP trace after visual inspection (Data: ',ARdataname,'; Data sampling: ',...
...
% end
% title(['Filtered LFP trace after visual inspection (Data: ',ARdataname,'; Data sampling: ',...
